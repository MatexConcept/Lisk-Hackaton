...
//       >Check Pool <svg className="woox-icon icon-arrow-right"> <use xlinkHref="#icon-arrow-right"></use>
...
const PoolInput = ({
  ...
  useEffect(() => {
    const loadTokenA = async () => {
      const token = await LOAD_TOKEN(token_A);
      if (token == undefined) {
        console.log("Kindly paste the token address for Token A");
      } else {
        setToken_1(token);
      }
    };
  ...
  useEffect(() => {
    const loadTokenB = async () => {
      const token = await LOAD_TOKEN(token_B);
      if (token == undefined) {
        console.log("Kindly paste the token address for Token B");
      } else {
        setToken_2(token);
      }
    };
  ...
  return (
    <>
      {token_1 ? (
        <Input
          value={`${token_1?.name} (${token_1?.symbol}) Bal: ${token_1?.balance ? token_1?.balance.slice(0, 8) : 'Loading...'}`}
        />
      ) : (
        <Input
          placeholder={"Token A"}
      ...
      {token_2 ? (
        <Input
          value={`${token_2?.name} (${token_2?.symbol}) Bal: ${token_2?.balance ? token_2?.balance.slice(0, 8) : 'Loading...'}`}
        />
      ) : (
        <Input
          placeholder={"Token B"}
      ...
      <button onClick={CALLING_POOL_Add} className="btn btn--large btn--green-light btn--with-icon btn--icon-right full-width">
        Check Pool
